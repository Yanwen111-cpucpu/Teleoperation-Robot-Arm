...
def rotation_3d(omega, theta):
    ...
    R=I+skew_3d(omega)/omega_n*math.sin(omega_n*theta)+skew_3d(omega)@skew_3d(omega)/(omega_n**2)*(1-math.cos(omega_n*theta))
    return R
...
def homog_3d(xi, theta):
    ...
    if np.all(omega==np.zeros(3)):
        g[0:3,0:3]=np.eye(3)
        g[0:3,3]=v.T*theta
        g[3,3]=1
    else:
        e_omega=rotation_3d(omega,theta)
        omega_n=np.linalg.norm(omega)
        omega_hat=hat_2d(omega)
       ...
       #omega=np.reshape(omega,(3,1))
        ...
        part_1=((np.eye(3)-e_omega)@(omega_hat@v))
        part_2=(omega[:,np.newaxis]@omega[np.newaxis,:]@v)*theta
        g[0:3,3]=((part_1+part_2)/(omega_n**2)).flatten()
        g[3,3]=1
    return g
